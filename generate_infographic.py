...
Flujo completo:
  ...
  4. Guarda el SVG en infographics/ y actualiza manifest.json
  5. Git add + commit + push → Vercel redeploy automático

Uso:
  python generate_infographic.py foto_hoy.jpg
  python generate_infographic.py foto_hoy.jpg --date 2025-03-20
  python generate_infographic.py foto_hoy.jpg --no-push   # solo genera, no publica
"""
...
# ── CONFIGURACIÓN ────────────────────────────────────────────────────────────
INFOGRAPHICS_DIR = Path(__file__).parent / "infographics"
...
def update_manifest(date_str: str, title: str, svg_filename: str, n_items: int):
    ...
    manifest.append({
        "date":  date_str,
        "title": title,
        "file":  f"infographics/{svg_filename}",
        "items": n_items,
    })
...
def git_push(date_str: str, svg_path: Path):
    ...
    cmds = [
        ["git", "add", str(svg_path), str(MANIFEST_FILE)],
...
def main():
    parser = argparse.ArgumentParser(description="Genera infográfico desde foto de papel")
    parser.add_argument("image", help="Ruta a la imagen (JPG/PNG/WEBP)")
    parser.add_argument("--date", default=datetime.today().strftime("%Y-%m-%d"),
                        help="Fecha YYYY-MM-DD (por defecto: hoy)")
    parser.add_argument("--no-push", action="store_true",
                        help="Solo genera el SVG, no publica en GitHub")
    ...
    # Paso 1: Extracción
    print("\n[1/4] Extrayendo contenido con Claude Vision...")
